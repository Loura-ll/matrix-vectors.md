...
flowchart TD
    ...
    A --> D[Transpose]
    A --> E[Identity and Zero]
    A --> F[Linear Systems]
    A --> G[Homogeneous Systems]
    A --> H[Invertibility]
    ...
    B --> B3[Column vector]
    B --> B4[Addition]
    B --> B5[Scalar multiplication]

    C --> C1[Inner dimensions match]
    C --> C2[Ax as linear combination]
    C --> C3[Columns of product]
    C --> C4[Rows of product]
    C --> C5[Associative not commutative]

    D --> D1[Rows become columns]
    D --> D2[Addition rule]
    D --> D3[Scalar rule]
    D --> D4[AB transpose equals BT AT]

    E --> E1[Zero matrix]
    E --> E2[Zero vector]
    E --> E3[Identity matrix]
    E --> E4[AI equals A]
    E --> E5[IA equals A]

    F --> F1[Augmented matrix]
    F --> F2[Row operations]
    F --> F3[RREF]
    F --> F4[Pivot variables]
    F --> F5[Free variables]
    F --> F6[No solution]
    F --> F7[Unique solution]
    F --> F8[Infinitely many solutions]

    G --> G1[Ax equals 0]
    G --> G2[n greater than m implies nonzero solution]

    H --> H1[Inverse matrix]
    H --> H2[Elementary matrices]
    H --> H3[Row operations as left multiplication]
    H --> H4[Product of elementary matrices]
    H --> H5[Invertible matrix equivalences]
    H --> H6[Find inverse by augmenting with I]
```
